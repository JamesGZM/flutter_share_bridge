## 0.1.0-dev.4

- Synchronized package versioning with the platform implementation callback hardening release.

## 0.1.0-dev.3

- Improved pub.dev metadata descriptions and public API documentation.

## 0.1.0-dev.2

- Updated pub.dev metadata and documentation links.
- Switched the public README to English and added a Chinese README entry.

## 0.1.0-dev.1

- Initial platform interface for Share Bridge federated plugins.
