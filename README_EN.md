...
This repository is still in local debugging stage and currently uses `path` dependencies internally. After publishing to pub.dev, host apps can depend on the packages they need:
...
The example app in this repository currently uses local path dependencies:
...
This version is still in real-device debugging and API stabilization stage. It has not been published to pub.dev yet.
